...
class ChildForm extends React.Component {
        ...
        if (
          ...
          result.push(
            <ListItem className="mb-1" key={key}>
              {key.replace('_', ' ')}: {data[key] && data[key].toString()}
            </ListItem>
          )
      }
      if (result.length)
  ...
  render() {
    const { medication, allergy, parent_contact, questionnaire, general_info, open } = this.state;
    const data = [];
    const infoArray = {
        ...
        <div key={key} className="d-flex justify-content-center">
          ...
          <Dialog open={openKey} onClose={(keyName) => this.handleClose(key)} aria-labelledby="simple-dialog-title">
            ...
            <div className="d-flex justify-content-center">
              <List>
              { !Array.isArray(infoArray[key]) && this.addToList(infoArray[key])}
              { Array.isArray(infoArray[key]) && infoArray[key].map((k) => this.addToList(k))}
              </List>
            </div>
          </Dialog>
        </div>
